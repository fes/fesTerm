...
| NP-05 | Review compact blue-graphite palette, active/inactive contrast, status semantics, icon legibility, all four bundled terminal families with ligatures off/on at supported scales, owned monochrome/color emoji fallback boundaries, and Inter application typography. Confirm Agency-style emoji remain aligned next to ASCII at each scale. | Visual + usability | Unicode 15.1 corpus tests, reviewed Windows/Linux snapshots, and scheduled three-platform native emoji smoke automate geometry and color-texture submission coverage; native pixel-level color appearance and scale judgment remain human |
