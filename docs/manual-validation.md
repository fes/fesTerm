...
| CP-06 | Validate the local native Markdown viewer against `docs/markdown-viewer-design.md`: command-palette open/cancel, Preview/Source toggle, Find, outline, reload/stale states, inert raw HTML/resource policy, explicit bounded local image loads, accessibility/focus, and return-to-prior-surface behavior. Remote SFTP preview remains deferred until the GUI SFTP browser lands. | Native functional + visual + accessibility + security | Yes for repository-owned local fixtures and error-state probes; retain human review for readability, focus, native picker, and screen-reader behavior |
