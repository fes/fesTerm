...
| NP-05 | Review compact blue-graphite palette, active/inactive contrast, status semantics, icon legibility, all four bundled terminal families with ligatures off/on at supported scales, owned monochrome/color emoji fallback boundaries, and Inter application typography. Confirm Agency-style emoji remain aligned next to ASCII at each scale. | Visual + usability | Deterministic glyph/raster/cache tests and reviewed snapshots automate geometry; native color appearance and scale judgment remain human |
