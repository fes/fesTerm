...
| Keyboard bindings and input recorder (#154) | Native macOS/Linux/Windows, non-US layouts, AltGr/Option/IME, terminal and widget focus | TI-10/TI-11 below; real selected fake-auth-URL Copy without submission, effective native menu mappings, reset/recovery and bounded redacted keyboard/mouse observations | Deterministic production dispatch/editor/config/mouse/privacy tests automated. Candidate native/layout/menu and usability evidence remains pending; macOS baseline driver blocked on Accessibility consent |
