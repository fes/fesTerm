...
| TI-11 | In a live SSH session with saved and ephemeral local/remote forwards, open Port Forward Manager from both its shortcut and palette entry; verify the list distinguishes saved-profile vs ephemeral state, add/remove loopback forwards, surface per-mapping failures without losing the shell, disconnect to clear the live list, and reconnect to confirm nothing is silently restored. | Native functional + usability | Partly: repository-owned SSH-fixture backend automation now covers the transport path, while overlay/shortcut wiring and native usability evidence remain pending |
...
| CP-06 | When the approved native Markdown viewer is implemented, validate its readability-first target, safe local/remote ownership, accessibility, resource isolation, bounds, and return-to-prior-surface behavior against `docs/markdown-viewer-design.md`. | Design approved; implementation validation deferred | No VM workflow until a bounded implementation exists |
