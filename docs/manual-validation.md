...
| SSH interaction workflows | All supported platforms with repository-owned fixture plus controlled native UI | Host-key comprehension, authentication focus and secrets, disconnect/history behavior, conditional reconnect, error recovery | Partly automated at transport level; native UI validation deferred until the complete interaction slice exists |
