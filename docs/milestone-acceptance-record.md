...
| P6 — ligature/fallback contract | In progress | ADR 0012 establishes cell geometry as the authority for glyph spans, cursor, selection, and hit testing. The initial width-two geometry regression is implemented; combining/fallback mapping tests and cross-platform snapshots remain before ligatures can be enabled. |
