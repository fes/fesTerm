...
| P4 — native platform smoke | In progress | Merged #15 supplies Windows-executed real PTY/ConPTY timing and shutdown coverage. The flow now validates an inbox fallback baseline before staging a hash-verified pinned ConPTY runtime for the production eframe/winit smoke. Its content-free probe records resize generations, output-byte deltas, CSI `6n` recognition, and nonblank-cell counts. First staged Windows CI, cross-platform confirmation, and independently driven platform-native focus/accessibility evidence remain. |
...
GitHub Copilot CLI, `less`, `vim`/`nvim`, `htop`, `tmux`, and shell line-editor
scenarios are **not run**. Record observer, date, platform, result, and a
minimal deterministic regression for every reproducible failure in
