...
| D — Visual snapshot layer | [#7](https://github.com/fes/fesTerm/issues/7) (closed) | Implemented; Linux/Windows/macOS baselines and CI matrix committed and **passing on all three platforms** as of `b8a242a`; fresh native re-run against `59980f9` still pending |
...
- [ ] Stable Windows, macOS, and Linux snapshot results and controlled diff artifacts (D/P3). Both platform baselines and the full scenario suite are committed and CI-wired, and CI now passes on all three platforms as of `b8a242a` ([#7](https://github.com/fes/fesTerm/issues/7) closed); a fresh **native-window** confirmation against the `59980f9` candidate (real hardware/VM run, not CI) is still needed before this can be checked off.
