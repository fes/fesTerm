...
| Windows, Linux, macOS | `native_daemon_survives_launcher_and_supports_input_replay_and_takeover` | **Executed 2026-08-27 at `79fbadc`**. The smoke requires explicit child-environment propagation, resize-before-replay on two geometry-changing reconnects, sustained 512 KiB output bursts, and takeover while the prior client is backpressured. Fresh optional-validation passed from repaired snapshots on macOS (run `20260827T221759Z-macos-festerm-160a5ff6-72da-4369-a85a-9a0ac3634b09`), Linux (run `20260827T224857Z-linux-festerm-0716c210-6e78-43df-819a-746e3ba9629d`), and Windows (run `20260828T202853Z-windows-festerm-2669a82b-6349-4963-887d-295715a8b0b2`). |

The Windows VM OS-input check now provides repeatable diagnostic evidence. A
...
manifest, and has no guest-input fallback. The final plan uses one acceptance
stage: host click, bounded focus settle, paced key presses, and an exact
Tab/Up/token PTY oracle that ignores OS key-repeat events. Clean-snapshot run
`20260828T202443Z-windows-festerm-0bc6ff9f-31a0-4eab-b6d5-bcca67b19a48`
passed the targeted check, and the full optional run above passed the same
path. During the
