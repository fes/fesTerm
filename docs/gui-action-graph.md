...
| `PROF-06` | Local profile edit → saved → launched | Enable tmux/screen and enter a validated session name on a saved Local profile. | Only that profile launches the direct attach-or-create provider command; built-in Local Shell remains a fresh plain shell and never inherits another terminal application's restoration identity/session ID. | Disable persistence and relaunch the saved profile; close disposable provider session. | P,H,N,U |
