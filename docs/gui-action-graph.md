...
| `SFTPG-02` | `GuiSftpReady → GuiSftpReady/TransferDrawer` | Queue uploads or downloads from the center rail while the remote side is connected and the destination is writable. | Transfers are delegated to `SftpTransferManager`; the drawer appears only while work exists, shows aggregate/item progress and per-item state, and canceling one item or batch does not halt unrelated queued work. | Clear or close completed work; retry failures individually. | P,H,N,U; partial |
...
| `WIN-06` | Any → update check/result | In a packaged build, press Check for Updates; download and install require separate later actions. | Fixed GitHub Releases endpoint and transmitted-data policy disclosed; signed artifacts only; quiet dismissible factual states; package manager respected; developer/incomplete builds have no network action; never blocks terminal/Launcher. | Close About; reset fixture endpoint/state. | P,H,N,U,privacy; partial |
