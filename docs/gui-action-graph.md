...
| `LAUNCH-09` | `K1 → SshDestination/GuiSftpAuth` | Activate SFTP. | The SFTP destination form reuses SSH destination validation and defaults to the graphical file manager; clearing **Use graphical file manager** launches the terminal file-transfer session. Saved SFTP profiles preserve this choice and ordinary SSH profiles retain their SFTP secondary action. | Back/Escape to `K1`, continue through `SSH-09`, or continue through `SFTPG-01`. | P,H,N,U; partial |
| `LAUNCH-10` | `K1/K9/Inspector → GuiSftpAuth` | Activate graphical SFTP from a saved SFTP/SSH profile, the command palette, or Session Inspector. | GUI SFTP opens as its own application surface with separate close lifecycle from any shell tab, preserving Local/Remote wording even when visual pane order later swaps. Current implementation re-prompts for credentials rather than yet reusing an in-memory authenticated SSH transport; persisted host trust is resolved from current configuration when the surface opens. | Cancel back to the prior surface or continue through `SFTPG-01`. | P,H,N,U; partial |
...
| `AUTH-04` | Stored-credential states (password or private key, ADR 0024) | Exercise available, missing, locked, unsupported, and backend-failed native store. | Only an opaque reference plus its kind persists; actionable non-secret feedback; one-off form never offers persistence. | Clear disposable store and return to `K1`. | P,H,N,U,privacy |
