...
| `LAUNCH-02` | `K1 → SshDestination` | Activate the SSH launch card. | The remote terminal matches Local's optical size; its accent globe retains two parallels and a meridian with proportional strokes at card and profile-row sizes. Same Launcher chip opens Quick Connect with initial focus on `user@host[:port]`; advanced destination fields preserve Username, Host, then Port order. Both surfaces share the same initially-off durable-session control. Advanced launch also accepts validated local/remote port forwards and attaches them to the initial session; no dimensions, TERM, or unsupported auth controls. | `LAUNCH-03`. | H,V,N,U |
...
| `LAUNCH-12` | `K1 → K1` | Enable "Resume unattached local sessions from New Session" (`SET-08`), then open New Session while locally running fesTerm-sessiond, tmux, and/or GNU screen sessions exist. | The Running Sessions panel groups entries under **fesTerm Native (sessiond)**, **tmux**, and **screen**, each with a disclosure header, count badge, and Reattach rows. fesTerm-sessiond rows dispatch `AppCommand::ResumeUnattachedSession`. tmux and screen rows dispatch `AppCommand::ResumeMultiplexerSession` with provider, exact match key, and display name. Invalid multiplexer names are omitted; already-attached tmux/screen sessions remain offered and annotated because those providers support multiple clients. Empty provider groups are omitted; no multiplexer absence error is surfaced. | Toggle `SET-08` back off, or rebuild `K1`. | P,H,N |
...
| `LAUNCH-20` | `RunningSessions → RunningSessions` | Activate the Running Sessions **Refresh** control. | The control requests a repaint so the composition root can supply refreshed session lists; it dispatches no `AppCommand` and does not mutate saved profiles or session definitions. | Already on Launcher. | H,N; deferred |
| `LAUNCH-21` | `PopulatedLauncher → PopulatedLauncher` | Open New Session at a width that keeps both panels side by side with more saved profiles or running sessions than fit their panel. | The launch card row and both panel footers keep their positions; the profile table and the session groups scroll inside their own panels rather than scrolling the whole surface. The surface stays inset from the window's content edge. The local density experiment keeps headings/search aligned and icon actions at least 24 px across; all five card titles fit without elision in a 1000 px window in both density modes. Below the stacking width the surface scrolls as one, because neither panel then has a bounded height to scroll inside. | Already on Launcher. | P,H,N |
...
| `TERM-02` | `K2/K4` | Drag/right-click/middle-click with mouse reporting off/on; repeat with Shift. | Ordinary ownership follows mode; Shift always forces complete local selection/context gesture/paste; never half a press/release; middle-click pastes locally when not claimed by mouse reporting. | Escape menu; clear selection by controlled click/type. | P,H,N |
| `TERM-03` | Selected primary/history text | Copy via shortcut, menu, native Edit; Copy command with no selection in a live session. | Soft wraps join, hard breaks remain, trailing cells omitted, Unicode correct; clipboard not logged; a copied selection clears afterward; a Copy command with no selection in a live session forwards the terminal interrupt character instead (the pinned windowing backend cannot distinguish plain Ctrl+C from Ctrl+Shift+C before either reaches the app). | Replace clipboard with empty fixture and clear selection. | P,H,N,privacy |
...
| `PROF-01` | Profiles → edit | Create/edit local and SSH definitions from the standalone Profiles surface. | Transport-specific launch fields; multi-field staging; validation local; running sessions unchanged. | Cancel discards draft. | P,H,V,N |
...
| `QUIT-01` | Multiple live sessions → aggregate modal | Close the fesTerm window (close button, native Quit menu, or Cmd+Q — fesTerm has exactly one window per process, so all three surface the same OS close request). | Exact local/SSH/serial counts, unsaved-history consequence, one modal, Cancel focused, action says Quit fesTerm. | Cancel returns exact window state; close request is not re-cancelled a second time. | P,H,V,N,U |
