...
| `LAUNCH-02` | `K1 → SshDestination` | Activate the SSH launch card. | The remote terminal matches Local's optical size; its accent globe retains two parallels and a meridian with proportional strokes at card and profile-row sizes. The SSH launcher opens on the separate destination fields in Username, Host, then Port order with initial focus on Username, and a heading toggle switches to the single `user@host[:port]` notation (the SFTP launcher's default) and back, carrying the destination across; exactly one notation is visible at a time. The SSH launcher, SFTP launcher and Profiles SSH/SFTP editor render the same destination pane. Both surfaces share the same initially-off durable-session control. Advanced launch also accepts validated local/remote port forwards and attaches them to the initial session; no dimensions, TERM, or unsupported auth controls. | `LAUNCH-03`. | H,V,N,U |
