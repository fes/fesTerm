...
| `TERM-08` | Any retained buffer | Open terminal context menu over selection/link/plain text and live/read-only state. | Exact applicable ordering: detected-path action (when resolvable), link actions, Copy, Paste, Find; unavailable entries omitted; disabled path actions must explain missing cwd/trust/auth metadata instead of silently doing nothing; no session/global actions or Select All. Selection survives. | Escape/outside dismiss; focus restored. | H,V,N |
