...
| `SET-06` | Settings → compact New Session grid toggled | Toggle "Compact multi-column New Session list" in the Interface card, then open New Session with several saved profiles and a window wide enough for more than one card. | Off by default; when on and wide enough, saved profiles lay out in a responsive multi-column grid instead of one per row, reducing scrolling for accounts with many profiles; Local Shell/New SSH/New Serial entries always stay single-column; falls back to single-column automatically at narrow widths or when the preference is off; keyboard selection and edit-icon/launch behavior are unchanged; preference persists automatically and independently of other Settings cards. | Toggle the preference back off, or narrow the window. | P,H,N; partial |
...
| `SET-08` | Settings → show-resumable-sessions toggled | Toggle "Resume unattached local sessions from New Session" in the Interface card, then open New Session while a locally running, unattached `festerm-sessiond` session, a tmux session, or a GNU screen session exists. | Off by default; when on, the New Session/Launcher screen lists each locally running, unattached `festerm-sessiond` session, each locally running tmux session, and each locally running GNU screen session as one-click "Resume" entries in their own labeled "fesTerm sessions"/"tmux sessions"/"GNU Screen sessions" widgets (after the fixed new-session entries, before saved profiles); clicking one attaches to it and opens a new session tab. An already-attached `festerm-sessiond` session never appears (its daemon only supports one attached client at a time), while an already-attached tmux/screen session still appears, annotated "attached elsewhere", since both multiplexers natively support more than one attached client. When off, or when none of the three are available, the Launcher behaves exactly as it does today with no error surfaced; preference persists automatically and independently of other Settings cards. | Toggle the preference back off. | P,H,N; partial |
