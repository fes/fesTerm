...
| `LAUNCH-11` | `AnySurface → LocalFileSelection/same` | Open **Open Markdown File…** from More actions or press `Ctrl+O`/`Cmd+O` from any surface, then either cancel/Escape the picker or choose a `.md`/`.markdown` file. | The picker reuses the SFTP file manager's local-directory browsing widget (breadcrumbs, up/home/refresh navigation, sortable columns, item icons) instead of an OS-native file dialog; Markdown files are the only entries a double-click/Enter can pick, while folders still navigate. It opens in the directory the previous picker was browsing, or the user's home directory the first time; Home resolves through `HOME` then `USERPROFILE`, so it is the real home folder on Windows as well as Unix. Cancel/Escape leaves the prior surface untouched. A successful pick opens or focuses one runtime Markdown viewer tab for that local path and continues through `MD-02`; when the picker was opened from inside a Markdown viewer, that viewer is retargeted at the picked file in place (keeping its Preview/Source, outline and status-bar preferences and discarding the previous document's resource approvals) instead of a second tab appearing. No workspace/session/transcript state is created. | Cancel the picker or close the viewer to restore the prior surface/`K1`. | P,H,N,U |
...
| `MD-04` | `Viewer → Viewer` | Navigate headings, links, tables, tasks, code, Find, selection/Copy, reload, resize, scale, and accessibility traversal. | The outline remains a bounded vertical sidebar while Preview/Source owns the remaining independently scrollable viewport; behavior meets the chosen readability/fidelity oracle, link/resource policy is enforced, semantic navigation and scroll restoration are accurate, and content stays out of logs/diagnostics. | Clear Find/selection and restore scale/scroll fixture baseline. | P,H,V,N,U,privacy; partial |
