...
| `LAUNCH-11` | `AnySurface → LocalFileSelection/same` | Open the command palette action **Open Markdown File…** and either cancel the native picker or choose a `.md`/`.markdown` file. | Cancel leaves the prior surface untouched. A successful pick opens or focuses one runtime Markdown viewer tab for that local path and continues through `MD-02`; no workspace/session/transcript state is created. | Cancel the picker or close the viewer to restore the prior surface/`K1`. | P,H,N,U |
