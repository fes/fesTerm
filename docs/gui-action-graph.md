...
| `SET-09` | Settings → default-sftp-local-directory changed | Enter, clear, or reject the SFTP card's default local directory. | Blank clears the override; an existing directory is accepted and persists for future SFTP tabs only; a missing or non-directory path is rejected inline without mutating other settings. Live SFTP sessions keep their current local directory until changed with `lcd`. | Clear the field or restore the baseline directory. | P,H,N; target |
