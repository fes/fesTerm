...
| `LAUNCH-09` | `K1 → SshDestination` | Activate SFTP. | Same Launcher chip opens an SFTP destination form that reuses SSH destination/authentication fields and validation, but launches a text-mode file-transfer session instead of a shell. Saved SSH profiles may also appear as explicit SFTP launcher entries without altering the underlying profile definition. | Back/Escape to `K1`, or continue through `SSH-09`. | P,H,N,U; target |
| `LAUNCH-10` | `K1/K9/Inspector → GuiSftpAuth` | Activate Open SFTP from a saved SSH profile, the command palette, or Session Inspector. | GUI SFTP opens as its own application surface with separate close lifecycle from any shell tab, preserving Local/Remote wording even when visual pane order later swaps. Current implementation re-prompts for credentials and requires an already persisted host-trust record rather than yet reusing an in-memory trusted/authenticated SSH transport. | Cancel back to the prior surface or continue through `SFTPG-01`. | P,H,N,U; partial |
