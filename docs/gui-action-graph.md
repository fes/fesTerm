...
| `LAUNCH-10` | `K1/K9/Inspector → GuiSftpAuth` | Activate graphical SFTP from a saved SFTP/SSH profile, the command palette, or Session Inspector. | GUI SFTP opens as its own application surface with separate close lifecycle from any shell tab, preserving Local/Remote wording even when visual pane order later swaps. Persisted host trust is resolved from current configuration when the surface opens; saved profiles may use their opaque native-store password/private-key reference or enter a fresh credential. | Cancel back to the prior surface or continue through `SFTPG-01`. | P,H,N,U; partial |
