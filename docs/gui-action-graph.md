...
| `PASTE-05` | `K12 → K2` | Immediate Enter, Escape, Cancel, outside click. | An Enter already waiting for clipboard remains behind the barrier, never activates a new dialog; asynchronous opening-frame controls are inert. After that frame, Enter targets Cancel until focus is deliberately moved. Escape/Cancel discard waiting keys with feedback; backdrop does not dismiss or touch terminal. | Already `K2`; reopen with risky fixture. | H,N |
