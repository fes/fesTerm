...
| `SET-06` | Settings → compact New Session layout toggled | Toggle "Compact New Session layout" in the Interface card, then open New Session with saved profiles and running sessions. | Off by default; when on, the Launcher's top launch cards drop their descriptions and shrink so the Saved Profiles and Running Sessions panels start higher. Saved Profiles remains a table with one row per profile; the old responsive saved-profile grid is not restored. The persisted `compact_launcher_grid` key still round-trips for compatibility with existing configuration files, and the preference persists automatically and independently of other Settings cards. | Toggle the preference back off. | P,H,N; partial |
