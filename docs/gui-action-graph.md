...
| `LAUNCH-02` | `K1 → SshDestination` | Activate the SSH launch card. | The card's mark renders large enough that its interior detail stays separable at the fixed stroke weight. Same Launcher chip opens Quick Connect with initial focus on `user@host[:port]`; advanced destination fields preserve Username, Host, then Port order. Both surfaces share the same initially-off durable-session control. Advanced launch also accepts validated local/remote port forwards and attaches them to the initial session; no dimensions, TERM, or unsupported auth controls. | `LAUNCH-03`. | H,V,N,U |
