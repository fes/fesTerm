...
The active tab must remain immediately distinguishable in both light and dark themes. Inactive tabs should be readable without competing visually with the active terminal. The application defaults to a dark theme (`egui::Visuals::dark()`, set in `app/festerm/src/app.rs`); a light theme is not yet exposed but the chip chrome derives most of its colors from `ui.visuals()` so it remains theme-complementary rather than hard-coded. All chips share the same neutral background fill; selection is indicated by outline color and the close control alone rather than by an accent fill or by highlighting the label text — this keeps the active/inactive distinction visible without relying on color-in-text alone or drawing attention away from terminal content.

Every chip, active or not, is drawn with a visible border: the active chip's border is a light grey (`Color32::from_gray(0xc8)`, 1.5px) and inactive chips use a darker grey (`Color32::from_gray(0x48)`, 1.0px), so the distinction reads clearly without color being the only cue. Each chip is two lines, left-aligned with a small reserved strip on the left for the status dot: the first line holds the status dot, the stable identity, and (only on the active chip) a close control; the second, smaller and muted line holds optional secondary terminal-provided metadata, indented under the first line's label rather than the chip's own left edge. The close control is only ever shown on the active chip — inactive chips never show it, even on hover — which keeps the row visually quiet and unambiguous about which chip's close action would fire. The "+ Launcher" control is painted with the same corner radius, border weight, and height as an inactive chip so it reads as part of the same family of controls.
