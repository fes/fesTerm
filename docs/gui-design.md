...
- `CHIP_INACTIVE_OUTLINE` (`Color32::from_gray(0x48)`, dark grey) — inactive chip borders, the "+ Launcher" control's border, and the drag-ghost preview outline.
- `CHIP_PRIMARY_TEXT` (`Color32::from_gray(0xe6)`) — the chip's primary (title) label and the "+ Launcher" label, always a light grey regardless of active/inactive state, so text is never hard to read.
