...
| E — Native platform smoke flows | [#8](https://github.com/fes/fesTerm/issues/8) | In progress (merged #15): PTY/`SessionController` timing layer implemented and executed on Windows (real ConPTY, issue #3 resize sequence, bounded shutdown all pass). Remaining scope: a real windowed (egui/winit) compositor, DPI, and native-focus smoke test — headless and PTY-layer tests cannot prove this. Linux/macOS PTY variants are written but unexecuted pending CI recovery. |
