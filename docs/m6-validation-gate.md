...
| E — Native platform smoke flows | [#8](https://github.com/fes/fesTerm/issues/8) | In progress: merged #15 provides the PTY/`SessionController` timing layer; an opt-in production eframe/winit self-smoke now drives the issue #3 resize sequence with controlled PTY I/O and observed native viewport metadata/focus. Cross-platform CI confirmation and independently driven platform-native focus/accessibility automation remain. |
