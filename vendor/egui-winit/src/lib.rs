...
impl State {
    ...
    fn on_keyboard_input(&mut self, event: &winit::event::KeyEvent) {
        ...
        if let Some(active_key) = logical_key.or(physical_key) {
            if pressed {
                // fesTerm integration: preserve exact per-event provenance before
                // semantic clipboard conversion (including an empty clipboard).
                // The app may consume the Key and its adjacent semantic event;
                // ordinary focused egui widgets still receive Copy/Cut/Paste.
                if is_cut_command(self.modifiers, active_key)
                    || is_copy_command(self.modifiers, active_key)
                    || is_paste_command(self.modifiers, active_key)
                {
            ...
            .or_else(|| winit_logical_key.to_text())
        {
            ...
            if !text.is_empty() && text.chars().all(is_printable_char) {
                ...
                let is_cmd = self.modifiers.mac_cmd
                    || ((self.modifiers.ctrl || self.modifiers.command) && !self.modifiers.alt);
                if pressed && !is_cmd {
                    self.egui_input
                        .events
