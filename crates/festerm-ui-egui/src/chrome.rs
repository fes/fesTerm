...
/// it to `f32` for `Painter::rect_filled`/`rect_stroke`, and both convert to
/// the same `CornerRadius` value (`CornerRadius::from(f32)` rounds to the
/// same `u8`).
const CHIP_CORNER_RADIUS: u8 = 6;
...
pub fn show(
    ...
    ui.horizontal(|ui| {
        ...
        let natural_widths: Vec<f32> = chips
            .iter()
            .map(|chip| chip::natural_chip_width(ui, chip, chip.id == active))
            .collect();
                ...
                ChipLayout::SingleRowScroll => {
                    if allocation.scrolling {
                        ui.with_layout(Layout::left_to_right(Align::Min), |ui| {
                            ...
                            let scroll_right =
                                controls::paint_chip_scroll_control(ui, true, chip_row_height);
